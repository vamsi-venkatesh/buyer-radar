...
| Model answers | `llm_cache` | content-addressed: the key already carries provider, model, prompt version and an input hash, so the row is keyed on it alone. Cost in rupees and token counts are kept per row. |
